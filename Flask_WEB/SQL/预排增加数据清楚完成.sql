INSERT INTO [dbo].[pbCommonDataExpectProduceDtl](sEquipmentPrepareNo,sCardNo,nRowNumber,tExpectDay)
SELECT *FROM (
SELECT sEquipmentPrepareNo,sCardNo ,ROW_NUMBER() OVER(PARTITION BY sEquipmentPrepareNo ORDER BY nRowNumber) AS nRowNumber2
,GETDATE() AS tExpectDay
FROM [198.168.6.236].[YYLT].dbo.pbCommonDataExpectProduceHdr
)A
WHERE A.nRowNumber2 <=5 AND sEquipmentPrepareNo <> 'wait'
...
----------清除不在列表中的数据
DELETE [198.168.6.236].[YYLT].dbo.pbCommonDataExpectProduceHdr
WHERE sCardNo NOT IN (SELECT sCardNo FROM #TEMP) AND sCardNo NOT IN ('故障','洗缸')
