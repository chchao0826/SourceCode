...
def toExcel():
    ...
    worksheet1 = workbook.add_sheet('染色作业日报%s' %thisDate)
    ...
    excelTitleVar = excelTitle()
    for i in range(len(excelTitleVar)):
        worksheet1.write(0, i, excelTitleVar[i], style)
    ...
    for i in listVar:
        b = 0
        for key in i:
            strLen = len(str(i[key]))
            # print(str(strLen) + '----' + str(i[key]))
            if strLen <= 8 and worksheet1.col(b).width != 5100:
                worksheet1.write(a, b, i[key], style)
                worksheet1.col(b).width = 3100
            else:
                worksheet1.write(a, b, i[key], style)
                worksheet1.col(b).width = 5100
            b += 1
        a += 1
