...
import os
from toExcel import pyxlToExcel, mergeExcel
...
# 删除昨日报表
def deleteLastTable():
    lastTableName = '染色作业日报%s' %getLastDate()
    isLastFile = os.path.isfile("%s.xlsx" %lastTableName)
    if isLastFile:
        os.remove("%s.xlsx" %lastTableName)
...
def emailMessage(att):
    ...
    subject = '自动发送邮件-%s染色作业日报 ' %nowDate
    message['subject'] = Header(subject, 'utf-8')
    message.attach(MIMEText('附件为%s染色作业日报,请查阅' %nowDate,'plain', 'utf-8'))
    message.attach(att)
    return message
...
def sendFile():
    nowDate = getDate()
    tableName = '染色作业日报%s' %nowDate
    att = MIMEText(open('%s.xlsx' %tableName, 'rb').read(), 'base64', 'utf-8')
    att['Content-Type'] = 'application/octet-stream'
    att['Content-Disposition'] = 'attachment; filename="%s.xlsx"' %nowDate
    return att
# 运行文件
